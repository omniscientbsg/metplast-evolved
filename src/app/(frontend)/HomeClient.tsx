...
const FEATURE_ICONS = [Bird, Award, Handshake];

export function HomeClient({ hero, content }: { hero: PageContentView; content: HomeContent }) {
    ...
    <main ref={containerRef} className="relative min-h-screen bg-[var(--bg)] overflow-hidden">
      ...
      >
        ...
        <motion.div style={{ scale, filter: `blur(${heroBlur}px)` }} className="w-full max-w-[1600px] mx-auto mt-20">
          ...
          <div className="lg:col-span-4 flex flex-col items-start lg:items-end gap-6 justify-end">
            ...
            >
              <div>
                <p className="text-4xl font-['Space_Grotesk'] font-black text-white">{content.heroStats[0].top}</p>
                {content.heroStats[0].bottom && <p className="text-xs font-bold text-white/70 uppercase tracking-widest mt-2">{content.heroStats[0].bottom}</p>}
              </div>
              <div>
                <p className="text-4xl font-['Space_Grotesk'] font-black text-white">{content.heroStats[1].top}</p>
                {content.heroStats[1].bottom && <p className="text-xs font-bold text-white/70 uppercase tracking-widest mt-2">{content.heroStats[1].bottom}</p>}
              </div>
              <div>
                <p className="text-lg font-['Space_Grotesk'] font-black text-white leading-tight">{content.heroStats[2].top}</p>
                {content.heroStats[2].bottom && <p className="text-xs font-bold text-white/70 uppercase tracking-widest mt-2">{content.heroStats[2].bottom}</p>}
              </div>
              <div>
                <p className="text-lg font-['Space_Grotesk'] font-black text-white leading-tight">{content.heroStats[3].top}</p>
                {content.heroStats[3].bottom && <p className="text-xs font-bold text-white/70 uppercase tracking-widest mt-2">{content.heroStats[3].bottom}</p>}
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="relative z-20 bg-[var(--bg)] pt-12 pb-6 px-6 -mt-8 transition-colors duration-500">
        <div className="max-w-[1600px] mx-auto grid md:grid-cols-3 gap-6">
          {content.featureCards.map((c, i) => {
            const Icon = FEATURE_ICONS[i] ?? Bird;
            return (
              <div key={i} className="bg-[var(--surface)] p-10 rounded-[2rem] border border-[var(--border)] relative overflow-hidden group shadow-xl transition-colors duration-500">
                <div className="absolute top-4 right-8 text-8xl font-black text-[var(--text)] opacity-[0.03] select-none pointer-events-none group-hover:scale-110 transition-transform duration-700">0{i+1}</div>
                ...
                <h3 className="text-2xl font-bold text-[var(--text)] mb-4 relative z-10 group-hover:text-[var(--accent)] transition-colors">{c.title}</h3>
        ...
        <div className="max-w-[1600px] mx-auto space-y-20">
          ...
          <div className="max-w-4xl text-center mx-auto space-y-6">
             <h2 className="text-3xl md:text-5xl font-['Space_Grotesk'] font-black tracking-tighter text-[var(--text)]">
                {content.positioning.heading} <span className="text-gradient">{content.positioning.accent}</span>
             </h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
             {content.solutionCards.map((sol, i) => (
                 <Link href={sol.href} key={i} className="group relative rounded-[2rem] overflow-hidden aspect-square border border-[var(--border)] hover:border-[var(--border)] transition-colors shadow-2xl bg-slate-900">
                    <Image src={sol.image} alt={sol.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700 opacity-60 group-hover:opacity-90" sizes="(max-width: 1024px) 50vw, 25vw" />
      ...
      <section className="relative z-20 py-24 px-6 bg-[var(--bg-elevated)] rounded-[4rem] text-[var(--text)] shadow-[var(--shadow)] mt-[-2rem] overflow-hidden border border-[var(--border)]">
        <div className="max-w-[1600px] mx-auto text-center space-y-16">
          <div className="max-w-3xl mx-auto space-y-6">
             <span className="text-primary font-bold tracking-widest uppercase text-sm">{content.overview.eyebrow}</span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
             {content.overview.tiles.map((tile, i) => (
                <Link href={tile.href} key={i} className="glass-panel rounded-2xl p-8 flex flex-col items-center justify-center gap-4 hover:shadow-xl hover:-translate-y-1 transition-all group">
        ...
        <div className="max-w-[1400px] mx-auto relative z-10">
          <div className="text-center space-y-6 mb-20">
             ...
             <h2 className="text-5xl md:text-7xl font-['Space_Grotesk'] font-black tracking-tighter text-[var(--text)] drop-shadow-xl">
                {content.calculators.heading} <br className="md:hidden" />
                <span className="text-gradient-blue">{content.calculators.accent}</span>
             </h2>
             <p className="text-xl text-[var(--text-muted)] font-medium max-w-2xl mx-auto">
               {content.calculators.body}
             </p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-8 justify-center">
            {content.calculators.cards.map((calc, i) => (
              <Link href={calc.href} key={i} className="group relative glass-panel rounded-3xl p-6 transition-all duration-700 hover:-translate-y-2 flex flex-col items-center gap-6 overflow-hidden hover:shadow-[0_20px_60px_rgba(34,88,120,0.2)]">
                ...
                <div className="relative w-24 h-24 rounded-full overflow-hidden border-2 border-[var(--border)] shadow-lg">
                  <Image src={calc.image} alt={calc.title} fill className="object-cover group-hover:scale-110 transition-transform duration-700 z-10" sizes="96px" />
                </div>
           ...
           <div className="glass-panel rounded-[2.5rem] p-10 flex flex-col justify-between transition-colors relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-64 h-64 bg-[var(--accent)]/10 rounded-full blur-3xl pointer-events-none" />
               <div className="space-y-6 relative z-10">
                 ...
                 <h3 className="text-3xl font-black font-['Space_Grotesk'] text-[var(--text)]">{content.teasers[0].title}</h3>
                 <p className="text-[var(--text-muted)] font-medium">{content.teasers[0].desc}</p>
              </div>
              <Link href="/environmental-control" className="mt-12 inline-flex items-center text-[var(--accent)] font-bold tracking-widest uppercase text-sm hover:gap-4 transition-all gap-2 relative z-10">
                 {content.teasers[0].ctaLabel} <ArrowRight className="w-4 h-4" />
              </Link>
           </div>
           ...
           <div className="glass-panel rounded-[2.5rem] p-10 flex flex-col justify-between transition-colors relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-64 h-64 bg-[var(--brand-navy)]/15 rounded-full blur-3xl pointer-events-none" />
               <div className="space-y-6 relative z-10">
                 ...
                 <h3 className="text-3xl font-black font-['Space_Grotesk'] text-[var(--text)]">{content.teasers[1].title}</h3>
                 <p className="text-[var(--text-muted)] font-medium">{content.teasers[1].desc}</p>
              </div>
              <Link href="/gallery" className="mt-12 inline-flex items-center text-[var(--accent)] font-bold tracking-widest uppercase text-sm hover:gap-4 transition-all gap-2 relative z-10">
                 {content.teasers[1].ctaLabel} <ArrowRight className="w-4 h-4" />
              </Link>
           </div>
           ...
           <div className="glass-panel rounded-[2.5rem] p-10 flex flex-col justify-between transition-colors relative overflow-hidden group">
              <div className="absolute top-0 left-0 w-64 h-64 bg-orange-500/10 rounded-full blur-3xl pointer-events-none" />
               <div className="space-y-6 relative z-10">
                 ...
                 <h3 className="text-3xl font-black font-['Space_Grotesk'] text-[var(--text)]">{content.teasers[2].title}</h3>
                 <p className="text-[var(--text-muted)] font-medium">{content.teasers[2].desc}</p>
              </div>
              <button
              ...
              >
                 {content.teasers[2].ctaLabel} <ArrowRight className="w-4 h-4" />
              </button>
           </div>
          ...
          <div className="pt-8">
            <Link href={content.cta.href}>
              <Button className="h-20 px-12 rounded-full bg-[var(--bg)] text-[var(--text)] hover:opacity-90 font-black text-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] hover:scale-105 transition-all">
                {content.cta.buttonLabel} <ArrowRight className="ml-4 w-8 h-8" />
              </Button>
            </Link>
          </div>
        </div>
