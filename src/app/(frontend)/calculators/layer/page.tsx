...
export default function LayerCalculatorPage() {
  ...
  const calculateLayer = (values: Record<string, number>): CalculatorResult[] | { error: string } => {
    ...
    return [
      { title: 'Total Bird Capacity', value: totalBirds.toLocaleString(), highlight: true },
  ...
  const layoutView = layoutData && [3,4,5,6].includes(layoutData.tiers) ? (
    <div className="relative inline-block w-full text-center bg-white rounded-xl p-4">
      {/* 
        Using native img instead of next/image to easily handle relative scaling 
        without explicit width/height constraints that break absolute positioning.
      */}
      <img 
        src={`/images/Layer-T1${layoutData.tiers}.png`} 
  ...
  return (
    <div className="bg-[var(--bg)] min-h-screen">
      <CalculatorShell 
        ...
        description="Calculate exact architectural requirements, cage modules, and capacity for H-Type and S-Frame layer farms."
